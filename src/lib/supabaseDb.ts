...
interface DbPrecioTej      { id: number; tipo_tejido: string; precio_kg_soles: number }
...
interface DbMovComplemento { id: number; fecha: string; tipo: string; complemento_tipo: string; color_id: number; talla: TallaDB; cantidad: number; precio_unit: number; total_soles: number; stock_antes: number; stock_despues: number; corte_id: number | null; n_corte: string | null; producto_destino_id: number | null; proveedor_id: number | null; num_factura: string | null; responsable: string; notas: string | null }
...
const normalizeAsignacion = (a: any): import('../types').SeguimientoAsignacion => ({
  ...a,
  tarifaId: String(a.tarifaId ?? a.tarifa_id ?? ''),
  operarioId: a.operarioId != null ? String(a.operarioId) : '',
  operarioIds: Array.isArray(a.operarioIds) ? a.operarioIds.map((id: any) => String(id)) : undefined,
});
const toSeguimientoFila = (r: DbSeguimiento): SeguimientoFila => ({ id: String(r.id), corteId: String(r.corte_id), nCorte: r.n_corte, productoId: String(r.producto_id), fecha: r.fecha, colorId: String(r.color_id), talla: r.talla, cantidad: r.cantidad, asignaciones: ((r.asignaciones as any[] | null) ?? []).map(normalizeAsignacion), pctAvance: r.pct_avance ?? r.porcentaje_avance ?? 0, estado: r.estado, totalPago: r.total_pago });
...
const toMovComplemento = (r: DbMovComplemento): MovimientoComplemento => ({ id: String(r.id), fecha: r.fecha, tipo: r.tipo as import('../types').TipoMovimientoComplemento, tipoComplemento: r.complemento_tipo, colorId: String(r.color_id), talla: r.talla, cantidad: r.cantidad, precioUnit: r.precio_unit, totalSoles: r.total_soles, stockAntes: r.stock_antes, stockDespues: r.stock_despues, corteId: r.corte_id ? String(r.corte_id) : undefined, nCorte: r.n_corte ?? undefined, productoDestinoId: r.producto_destino_id ? String(r.producto_destino_id) : undefined, proveedorId: r.proveedor_id ? String(r.proveedor_id) : undefined, nFactura: r.num_factura ?? undefined, responsable: r.responsable, notas: r.notas });
...
const fromMovTelaInsert = (v: MovimientoTela) => ({ id: v.id, fecha: v.fecha, tipo: v.tipo, cliente_id: v.clienteId ? safeInt(v.clienteId) : null, tela_id: safeInt(v.telaId), color_id: safeInt(v.colorId), rollos: v.rollos, kg_total: v.kgTotal, categoria_color: v.categoriaColor, precio_kg: v.precioKg, total_soles: v.totalSoles, stock_rollos_antes: v.stockRollosAntes, stock_rollos_despues: v.stockRollosDespues, responsable: v.responsable, proveedor_id: v.proveedorId ? safeInt(v.proveedorId) : null, n_factura: v.nFactura ?? null, costo_real_fact: v.costoRealFact ?? null, corte_id: v.corteId ? safeInt(v.corteId) : null, n_corte: v.nCorte ?? null, notas: v.notas });
// fromCorte omite 'id' porque la PK es integer auto-generada; convierte FKs a integer
const fromCorte = (v: Corte) => {
  const base: Record<string, unknown> = { n_corte: v.nCorte, fecha: v.fecha, cliente_id: parseInt(v.clienteId), producto_id: parseInt(v.productoId), color_id: parseInt(v.colorId), tonalidad: v.tonalidad ?? null, colores_detalle: v.coloresDetalle ?? null, tela_id: v.telaId ? parseInt(v.telaId) : null, cortador: v.cortador, ayudante: v.ayudante, tendedor: v.tendedor, kg_usados: v.kgUsados, rollos_usados: v.rollosUsados, tendidas: v.tendidas, mts_por_tendida: v.mtsPorTendida, ancho_cm: v.ancho, cant_s: v.cantS, cant_m: v.cantM, cant_l: v.cantL, cant_xl: v.cantXL, total_prendas: v.totalPrendas, consumo: v.consumo, rendimiento: v.rendimiento, revision: v.revision === 'VERIFICADO', traslado: v.traslado, estado: v.estado, pago_cliente: v.pagoCliente === 'COBRADO' ? 1 : 0, pago_planilla: v.pagoPlanilla === 'PAGADO' ? 1 : 0, costo_mo_corte: v.costoMoCorte, notas: v.notas };
...
const fromMovComplemento = (v: MovimientoComplemento) => ({ id: v.id, fecha: v.fecha, tipo: v.tipo, complemento_tipo: v.tipoComplemento, color_id: safeInt(v.colorId), talla: v.talla, cantidad: v.cantidad, precio_unit: v.precioUnit, total_soles: v.totalSoles, stock_antes: v.stockAntes, stock_despues: v.stockDespues, corte_id: v.corteId ? safeInt(v.corteId) : null, n_corte: v.nCorte ?? null, producto_destino_id: v.productoDestinoId ? safeInt(v.productoDestinoId) : null, proveedor_id: v.proveedorId ? safeInt(v.proveedorId) : null, num_factura: v.nFactura ?? null, responsable: v.responsable, notas: v.notas });
const fromMovComplementoInsert = (v: MovimientoComplemento) => ({ fecha: v.fecha, tipo: v.tipo, complemento_tipo: v.tipoComplemento, color_id: safeInt(v.colorId), talla: v.talla, cantidad: v.cantidad, precio_unit: v.precioUnit, total_soles: v.totalSoles, stock_antes: v.stockAntes, stock_despues: v.stockDespues, corte_id: v.corteId ? safeInt(v.corteId) : null, n_corte: v.nCorte ?? null, producto_destino_id: v.productoDestinoId ? safeInt(v.productoDestinoId) : null, proveedor_id: v.proveedorId ? safeInt(v.proveedorId) : null, num_factura: v.nFactura ?? null, responsable: v.responsable, notas: v.notas });
...
async function dbInsert<T>(table: TableName, row: T, mapper: (v: T) => Record<string, unknown>, keepId = false): Promise<string | null> {
  const mapped = mapper(row);
  const payload = keepId ? mapped : (() => { const { id: _id, ...rest } = mapped; void _id; return rest; })();
...
export const db = {
  ...
  boletaLineas: {
    add: (v: BoletaLinea) => dbInsert('boleta_lineas', v, fromBoletaLineaInsert, true),
