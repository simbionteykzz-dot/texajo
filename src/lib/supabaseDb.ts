...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const toBoletaLinea = (r: any): BoletaLinea => ({ id: String(r.id), operarioId: String(r.operario_id), corteId: String(r.corte_id), nCorte: r.n_corte, productoId: String(r.producto_id), tarifaId: String(r.tarifa_id), operacion: r.operacion, orden: r.orden, tarifa: r.tarifa, cantPrendas: r.cant_prendas, importe: r.importe, periodo: r.periodo, fechaRegistro: r.fecha_registro ?? undefined, estadoPago: r.estado_pago, fechaPago: r.fecha_pago ?? undefined });
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
const fromOperario = (v: Operario) => ({ id: v.id, codigo: v.codigo, nombre_completo: v.nombre, estado: v.estado, dni: v.dni ?? null, telefono: v.telefono ?? null, modulo: v.modulo ?? null, maquina: v.maquina ?? null, fecha_ingreso: v.fechaIngreso ?? null });
